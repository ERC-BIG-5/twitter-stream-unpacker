...
from consts import CONFIG, STATUS_FILE, BASE_PROCESS_PATH, logger
...
def process_dump(dump_path: Path):
    ...
    logger.info(f"dumping: {dump_file_date}")
    dump_file_status = {}
        ...
        for jsonl_file_name, jsonl_file_data in iter_jsonl_files_data(tar_file):
            ...
            for jsonl_entry in (jsonlines.Reader(io.BytesIO(jsonl_file_data))):
                ...
                if "data" in jsonl_entry:
                    ...
                    data = jsonl_entry
                if (((data.get("referenced_tweets") is None and data.get(
                        "in_reply_to_status_id") is None
                      and data.get("quoted_status_id") is None and data.get(
                            "retweeted_status")) or not CONFIG.ONLY_ORIG_TWEETS)
            ...
            json_file_status.update(
                {"entries_count": entries_count,
                 ...
                 "latest_dt": latest_dt.strftime("%a %b %d %H:%M:%S %z %Y")
                 })
        break
